...
export interface DropdownProps {
    ...
    */
    ...
    /**
    * @member {boolean} [minWidth] The miniumum width for the input field of the component (in pixels)
    * @default false
    */
    ...
    */
    ...
    /**
    * @member {POSITIONS} [positionX] X-axis position
    * Not implemented
    */
    positionX: POSITIONS;

    /**
    * @member {POSITIONS} [positionY] Y-axis position
    * Not implemented
    */
    positionY: POSITIONS;
...
/**
 * List of possible dropdown position values
 * @readonly
 * @enum {string}
 */
enum POSITIONS {
...
const Dropdown = ({
    ...
    const [elementSize, setElementSize] = useState({ height: 24, width: 0 })
    ...
    return (
        <>
            <StyledDropdown
                ...
                ref={dropdownRef}
                style={fullWidth ? { width: '100%' } : minWidth ? { width: minWidth } : {}}
                onClick={toggleDropdown}>
                <span className='drui-dropdown-input-label'>
                    {!selected ? items[defaultIndex ? defaultIndex : 0].title : selected!.title}
                </span>
                <div className="drui-dropdown-arrow">
                    ↓
                </div>
                {
                    isOpen ?
                        <StyledDropdownList
                            className={
                                [
                                    'drui-dropdown',
                                    className && `${className}`,
                                    positionX && `drui-dropdown--${positionX}`,
                                    positionY && `drui-dropdown--${positionY}`,
                                ].join(' ')
                            }
                            onMouseLeave={handleMouseLeave}
                            style={{ top: `${elementSize.height}px`, left: '0px' }}>
                            <span
                                ...
                                >
                                    {
                                        ...
                                        items.map(
                                            (item, index) => {
                                                ...
                                                const onItemElemClick = (e: any) => {
                                                    if (item.onClick) item.onClick(item);
                                                    if (closeOnItemClick && item.closeDropdown) item.closeDropdown();
                                                    setSelected(item);
                                                    toggleDropdown();
                                                }
                                                return (
                                                    <li className="drui-dropdown__listItemWrapper"
                                                        ...
                                                        <StyledDropdownItem
                                                            className={[
                                                                'drui-dropdownItem',
                                                                className && `${className}` || '',
                                                                item.description && 'drui-dropdownItem--withDescription' || '',
                                                                item.active && 'drui-dropdownItem--active' || '',
                                                                item.disabled && 'drui-dropdownItem--disabled' || '',
                                                                item.showItemStatus && 'drui-dropdownItem--showItemStatus' || '',
                                                        ...
                                                        >
                                                            <ActionElem
                                                                ...
                                                                target={item.target}
                                                                rel={item.target === '_blank' ? 'noopener nofollow' : undefined}
                                                                disabled={item.disabled}
                                                                onClick={onItemElemClick}
                                                            >
                                                                <div className="drui-dropdownItem__iconWrapper">
                                                                    {/* Custom icon for dropdown item */}
                                                                    {item.Icon && <item.Icon className="drui-dropdownItem__icon" />}

                                                                    {/* No custom icon, menu item is active */}
                                                                    {!item.Icon && showItemStatus && item.active &&
                                                                ...
                                                                <div className="drui-dropdownItem__title">
                                                                    <span className="drui-dropdownItem__titleText">{item.title}</span>
                                                                ...
                                                                {/* Item description */}
                                                                {item.description &&
                                                                    <span className="drui-dropdownItem__description">{item.description}</span>
                                                                }
                                                            </ActionElem>
